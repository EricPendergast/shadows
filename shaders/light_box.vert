...
// Says which plane to project onto
/*uniform int plane;*/

layout(location = 0) in vec4 in_Position;

noperspective out float distance_inv;
out float distance;
out float in_range;
out vec4 rel_pos;
...
void main(void) {
    float plane_proj = in_Position.x/abs(in_Position.y);
    // TODO: w coordinate aeffects clipping. Maybe make it zero when
    // in_Position.y is negative?
